...
class Quantity(object):
    ...
    # 判断配对msku是否是新ASIN，如果不是则同步亚马逊图片，如果没有图片则标记MSKU未同步图片，是新ASIN则将ASIN表更新并标记MSKU未同步图片
    ...
    def batch_pair(self, path):
        ...
        row1 = wb_sheet.max_row
        for i in range(row1, 0, -1):
            cell_value1 = wb_sheet.cell(row=i, column=2).value
            if cell_value1:
                row1 = i
                break
        msg = True
        for i in range(2, row1+1):
            ...
            if fnsku and msku and asin and store and version:
                ...
                wb_sheet.cell(row=i, column=9).value = f"错误：请检查表格输入是否完整。"
                msg = False
        time_now = datetime.datetime.now().strftime("%Y%m%d%H%M")
        wb.save(f'./static/批量配对/批量配对详情_{time_now}.xlsx')
        return msg, [f"../批量配对/批量配对详情_{time_now}.xlsx", f"批量配对详情_{time_now}"]

    def get_pair_msg(self, path):
        wb = openpyxl.load_workbook(path)
        wb_sheet = wb.active
        row_max = wb_sheet.max_row
        ws = openpyxl.Workbook()
        ws_sheet = ws.active
        ws_sheet.append(['MSKU', '配对状态'])
        self.sql()
        for i in range(2, row_max+1):
            msku = wb_sheet.cell(row=i, column=1).value
            sql = "select * from `amazon_form`.`pre_msku` where `MSKU` = '%s'" % msku
            self.cursor.execute(sql)
            result = self.cursor.fetchall()
            if result:
                ws_sheet.append([msku, result[0]['状态']])
            else:
                ws_sheet.append([msku, '没有找到这个MSKU，请检查'])
        self.sql_close()
        time_now = datetime.datetime.now().strftime("%Y%m%d%H%M")
        ws.save(f'./static/批量配对/批量配对详情_{time_now}.xlsx')
...
class Find_order():
    ...
    # 批量配对 pre_msku 数据库的msku，下载listing表格获取ASIN、fnsku等信息，配对ASIN是否唯一再配对
    def get_msku(self):
        try:
            self.sql()
            ...
            if result:
                ...
                # print(self.dict_msku)
                # list_asin = self.read_excl('539742484006551552')
                # print(self.dict_msku)
                if list_asin:
    ...
    def update_listing(self):
        self.sql()
        sql = "select * from `data_read`.`listing` where `品名` = '品名数据未更新'"
        self.cursor.execute(sql)
        result = self.cursor.fetchall()
        if result:
            for i in result:
                product_name, supplier = self.get_product_name(i['SKU'])
                if product_name:
                    sql2 = f"update `data_read`.`listing` set `品名` = '{product_name}', `供应商` = '{supplier}' where `key` = '%s'" % i['key']
                    self.cursor.execute(sql2)
            self.connection.commit()
        self.sql_close()
    ...
    def get_product_name(self, sku):
        ...
        if result:
            supplier = "供应商数据未更新"
            if result[0]['供应商']:
                supplier = result[0]['供应商']
            return result[0]['品名'], supplier
        else:
            return "品名数据未更新", "供应商数据未更新"
